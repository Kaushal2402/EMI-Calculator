...
- [x] **9.1 App icon set (D-04)** — 1024px master → all Android mipmap densities + iOS @1x/@2x/@3x via `flutter_launcher_icons`.
  - **DONE 2026-09-06** (branch `feat/premium-ui-ads-icon`): master art generated by
    `scripts/gen_icon.py` (PIL) — vector rupee mark, white on the brand-blue
    Primary600→800 gradient, soft depth disc. Outputs `assets/icon/app_icon.png`
    (1024, opaque), `app_icon_foreground.png` + `app_icon_monochrome.png` (safe-zone
    glyph). `flutter_launcher_icons: ^0.14.4` config in `pubspec.yaml`
    (`min_sdk_android: 24`, adaptive bg `#0B4584` + foreground + Android-13
    monochrome). Ran `dart run flutter_launcher_icons`: all Android mipmaps +
    `mipmap-anydpi-v26/ic_launcher.xml` + `values/colors.xml` + `drawable-*`
    foreground/monochrome, all iOS sizes incl. opaque 1024, regenerated. iOS
    simulator build to eyeball on-device is blocked by a pre-existing
    `google_mobile_ads` 9.1 non-modular-header Pods issue (unrelated).
