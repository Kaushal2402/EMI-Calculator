...
- [x] **7.3 `AdmobBannerWidget`** — 320×50 pinned above system nav on Results; reserves space, never overlaps content (AC-07).
  ...
  - **UPDATE 2026-09-06** (branch `feat/premium-ui-ads-icon`, client ask
    "show ads at every possible place"): the same `AdmobBannerWidget` is now
    also mounted on the **Home / Calculator** screen via
    `AppScaffold.bottomNavigationBar`. `_CalculatorForm` dropped its trailing
    `SizedBox(kSpacing24 + bottomInset)` (banner owns the inset now) for a flat
    `kSpacing24`. New test `calculator_screen_test.dart › mounts the AdMob
    banner slot on Home`.
- [x] **7.4 Interstitial** — counter in persistent storage; show on every 5th calculation, never on first use; preload next.
  ...
  - **UPDATE 2026-09-06** (branch `feat/premium-ui-ads-icon`) — reliability +
    reach:
    * `InterstitialAdController` is now `ref.watch`ed in `app.dart`, so it is
      built at launch and starts preloading immediately instead of lazily on
      the first calculation — the first every-5th boundary meets a filled slot.
    * `_preload()` failures now schedule a bounded back-off retry
      (`_scheduleRetry`, ≤5× at 20 s) instead of waiting for the next boundary,
      so a flaky first fill (offline at launch / slow SDK init) self-heals.
      Retry counter resets on a successful load or a show. `dispose()` cancels
      the timer. `enabled:false` (tests) ⇒ still a full no-op, no timers.
    * Second presentation point: the **Results → back** `BackButton` also
      `await`s `maybeShowAtBoundary()` (a natural content boundary; still
      gated to every-5th so no extra frequency). Existing 5 controller tests
      unchanged and green.
...
- [x] **9.1 App icon set (D-04)** — 1024px master → all Android mipmap densities + iOS @1x/@2x/@3x via `flutter_launcher_icons`.
  - **DONE 2026-09-06** (branch `feat/premium-ui-ads-icon`): **client-supplied
    master art** `assets/icon/app_icon.png` (1254² opaque — 3D calculator +
    house + ₹ coin stack on the brand blue). `flutter_launcher_icons: ^0.14.4`
    in `pubspec.yaml` (`remove_alpha_ios: true`, `min_sdk_android: 24`,
    adaptive: white background + the art as foreground so its own white margin
    blends). `dart run flutter_launcher_icons` regenerated every Android mipmap
    + `mipmap-anydpi-v26/ic_launcher.xml` + `values/colors.xml` +
    `drawable-*/ic_launcher_foreground` and all iOS sizes incl. the opaque
    1024. (An earlier PIL-generated vector rupee was replaced by the client
    art.) iOS *simulator* build to eyeball on-device is still blocked by a
    pre-existing `google_mobile_ads` 9.1 non-modular-header Pods issue.

### 9.1b — Premium UI refresh + brand palette (client feedback, 2026-09-06)

- [x] **Brand palette** — client colours applied to `AppColors` (both schemes):
  Primary `#0060D0`, Deep `#0030A0`, Bright/CTA `#0080F0`, Navy text `#203050`,
  Gold `#F0D040`, White. All AC-09 pairs re-audited green in
  `test/core/wcag_contrast_test.dart` (the gold "interest" swatch is delimited
  by an `onSurfaceVariant` hairline to hold WCAG 1.4.11 on light surfaces).
  Donut interest segment is now gold, not the old alarming red.
- [x] **Bundled fonts** — Poppins + Inter vendored under `assets/fonts/`
  (OFL 1.1, see `assets/fonts/README.md`) and declared in `pubspec.yaml`;
  `AppTypography` switched from `google_fonts` runtime loading to
  `fontFamily:`. First frame no longer waits on a network font; goldens now
  render real glyphs (retires the "block glyph" caveat, qa P2-01/P2-04).
  `test/flutter_test_config.dart` loads them via `FontLoader`.
- [x] **Calculator screen** — inputs grouped in one card; a live "ESTIMATED
  MONTHLY EMI" chip recalculates as you edit; CALCULATE EMI is a pinned
  bottom `GradientButton` (bright→deep-blue) above the banner slot; sliders
  lose the busy tick dots (`SliderThemeData`), text fields go borderless-filled
